...
// Copyright (c) 2011-2013 The YOBSY developers
...
const string strMessageMagic = "YOBSY Signed Message:\n";
...
void static SyncWithWallets(const CTransaction& tx, const CBlock* pblock = NULL, bool fUpdate = false, bool fConnect = true)
{
    if (!fConnect)
    {
        // YOBSY: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
...
bool CTransaction::CheckTransaction() const
{
    ...
    for (int i = 0; i < vout.size(); i++)
    {
        ...
        // YOBSY: enforce minimum output amount
                        ...
                        bool* pfMissingInputs)
{
    ...
    // YOBSY: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// YOBSY: find block wanted by given orphan block
...
// YOBSY: miner's coin stake is rewarded based on coin age spent (coin-days)
...
// YOBSY: find last block index up to pindex
...
unsigned int static GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // YOBSY: target change every block
    // YOBSY: retarget with exponential moving toward target spacing
    CBigNum bnNew;
...
bool IsInitialBlockDownload()
{
    ...
    return (GetTime() - nLastUpdate < 10 &&
            pindexBest->GetBlockTime() < GetTime() - 60 * 60);
}
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    // YOBSY: should not enter safe mode for longer invalid chain
}
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // YOBSY: check transaction timestamp
        ...
        if (IsCoinStake())
        {
            // YOBSY: coin stake tx earns reward instead of paying fee
            uint64 nCoinAge;
        ...
        else
        {
            ...
            // YOBSY: enforce transaction fees for every block
            if (nTxFee < GetMinFee())
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // YOBSY: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // YOBSY: track money supply and mint amount info
    ...
    // YOBSY: fees are not collected by miners as in bitcoin
    // YOBSY: fees are destroyed to compensate the entire network
    if (fDebug && GetBoolArg("-printcreation"))
...
// YOBSY: total coin age spent in transaction, in the unit of coin-days.
...
// YOBSY: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
    ...
    // YOBSY: compute chain trust score
    pindexNew->bnChainTrust = (pindexNew->pprev ? pindexNew->pprev->bnChainTrust : 0) + pindexNew->GetBlockTrust();

    // YOBSY: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // YOBSY: record proof-of-stake hash value
    ...
    if (!CheckStakeModifierCheckpoints(pindexNew->nHeight, pindexNew->nStakeModifierChecksum))
        return error("AddToBlockIndex() : Rejected by stake modifier checkpoint height=%d, modifier=0x%016"PRI64x", checksum=%u", pindexNew->nHeight, nStakeModifier, pindexNew->nStakeModifierChecksum);
...
bool CBlock::CheckBlock() const
{
    ...
    // YOBSY: only the second transaction can be the optional coinstake
    for (int i = 2; i < vtx.size(); i++)
        if (vtx[i].IsCoinStake())
            return DoS(100, error("CheckBlock() : coinstake in wrong position"));

    // YOBSY: coinbase output should be empty if proof-of-stake block
    ...
    if (vtx[0].GetValueOut() > (IsProofOfWork()? (GetProofOfWorkReward(hashPrevBlock) - vtx[0].GetMinFee() + MIN_TX_FEE) : 0))
        return DoS(50, error("CheckBlock() : coinbase reward exceeded %s > %s", 
                   FormatMoney(vtx[0].GetValueOut()).c_str(),
                   FormatMoney(IsProofOfWork()? GetProofOfWorkReward(hashPrevBlock) : 0).c_str()));
...
bool CBlock::AcceptBlock()
{
    ...
    // YOBSY: check that the block satisfies synchronized checkpoint
    ...
    // YOBSY: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // YOBSY: verify hash target and signature of coinstake tx
    ...
    // YOBSY: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // YOBSY: getblocks may not obtain the ancestor block rejected
    ...
    // YOBSY: if responsible for sync-checkpoint send it
...
// YOBSY: entropy bit for stake modifier if chosen by modifier
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < (uint64)15000000 + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        ThreadSafeMessageBox(strMessage, "YOBSY", wxOK | wxICON_EXCLAMATION | wxMODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "YOBSY", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // YOBSY: initialize synchronized checkpoint
        if (!Checkpoints::WriteSyncCheckpoint(hashGenesisBlock))
            return error("LoadBlockIndex() : failed to init sync checkpoint");

        // YOBSY: upgrade time set to zero if txdb initialized
        {
            CTxDB txdb;
    ...
    // YOBSY: if checkpoint master key changed must reset sync-checkpoint
    {
        CTxDB txdb;
        string strPubKey = "";
...
static string strMintMessage = _("Info: Minting suspended due to locked wallet. To unlock wallet, use walletpassphrase in the Debug Console"); 
...
string GetWarnings(string strFor)
{
    ...
    // YOBSY: wallet lock warning for minting
    if (strMintWarning != "")
    {
        nPriority = 0;
    ...
    // YOBSY: should not enter safe mode for longer invalid chain
    // YOBSY: if sync-checkpoint is too old do not enter safe mode
    if (Checkpoints::IsSyncCheckpointTooOld(60 * 60 * 24 * 10) && !fTestNet)
    {
        nPriority = 100;
        strStatusBar = strRPC = "The blockchain is downloading and verifying blocks; do not create any new transactions until you get a Thumbs Up in the bottom right corner";
    }

    // YOBSY: if detected invalid checkpoint enter safe mode
    if (Checkpoints::hashInvalidCheckpoint != 0)
    {
        nPriority = 3000;
        strStatusBar = strRPC = "WARNING: Invalid checkpoint found! If this alert continues, please email support@gorillastake.com";
    }

    // YOBSY: if detected unmet upgrade requirement enter safe mode
    ...
    // Alerts
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CAlert)& item, mapAlerts)
        {
            ...
            if (alert.AppliesToMe() && alert.nPriority > nPriority)
            {
                ...
                if (nPriority > 1000)
                    strRPC = strStatusBar;  // YOBSY: safe mode for high alert
            }
        }
    }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        // YOBSY: record my external IP reported by peer
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    if (inv.hash == pfrom->hashContinue)
                    {
                        ...
                        // YOBSY: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // YOBSY: tell downloading node about the latest block if it's
...
CBlock* CreateNewBlock(CReserveKey& reservekey, CWallet* pwallet, bool fProofOfStake)
{
    ...
    // YOBSY: if coinstake available add coinstake tx
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // YOBSY: simplify transaction fee - allow free = false
    ...
    if (pblock->IsProofOfWork())
        pblock->vtx[0].vout[0].nValue = GetProofOfWorkReward(pblock->hashPrevBlock);
...
void BitcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        if (fProofOfStake)
        {
            // YOBSY: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake())
            {
